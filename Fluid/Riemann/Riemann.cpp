...
FlowFlux Riemann::HLLC_Χ(BaseVar& CL, BaseVar& CR, coordTrans& ct)//半点左侧右侧格点，坐标变换参考点
{
	double ξx = ct.ξx();
	double ξy = ct.ξy();
	double ξt = ct.ξt();
	...
	if (SL > 0)
		...
		F_HLLC = FR;
	F_HLLC.changeFF(F_HLLC.flux(0) * Dξ, F_HLLC.flux(1) * Dξ, F_HLLC.flux(2) * Dξ, F_HLLC.flux(3) * Dξ);
	return F_HLLC;
}
FlowFlux Riemann::HLLC_Y(BaseVar& CD, BaseVar& CU, coordTrans& ct)//半点左侧右侧格点，坐标变换参考点
{
	double ηx = ct.ηx();
	double ηy = ct.ηy();
	double ηt = ct.ηt();
	...
	G_HLLC.changeFF(G_HLLC.flux(0) * Dη, G_HLLC.flux(1) * Dη, G_HLLC.flux(2) * Dη, G_HLLC.flux(3) * Dη);
