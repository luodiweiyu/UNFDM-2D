...
/*网格初始化，根据参数生成计算网格|后期可以更改内部规则，从文件读入或其他等
* 1.从计算域左下角（xmin,ymin）开始先往整个计算域布满正六边形
* 2.每次添加正六边形时，需要判定此六边形在流场中的位置
*			若六个点均在流场内部，则直接将该六边形当作网格单元插入网格中去
*			若部分点在流场外部，则需要将该六边形进行切割，再将切割后的多边形插入网格中
*/
void Mesh::init()
{
